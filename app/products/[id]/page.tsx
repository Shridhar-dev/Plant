...
function ProductPage({params}:{params:any}) {
  ...
  const getProduct = async () => {
    ...
    setProduct({...objProduct.product, images: objProduct.images, reviews: objProduct.reviews})
  ...
  const addReview = async() => {
    ...
    await fetch('/api/review', {
          ...
          body: JSON.stringify({review, productId: product?.id, id:user ? user.id : 1}),
          })
          ...
          <div className='flex items-center gap-3 mt-4'>
              {
                product?.images.map((imageObj:{image:string}, i:number)=>(
                  <FocusImage key={i} image={imageObj.image} setFocusImage={setCurrentProductImage}/>
                ))
              }
          </div>
        </div>
        <div className='col-span-5 lg:col-span-3 flex h-screen flex-col gap-3 pt-10 lg:overflow-auto scrollbar-hide'>
          ...
          <div className='mt-10'>
            ...
            <div className='mb-5 mt-2 w-2/3 ml-1 flex items-center gap-2'>
              ...
              <Button variant="secondary" onClick={addReview}>Submit</Button>
            </div>
            <div className='flex flex-col gap-5 py-2'>
              {
                product?.reviews?.map((review, id)=>(
                  <Review key={id} review={review.review} image={review.user_image} name={review.user_name} />
                ))
              }
            </div>
          </div>
